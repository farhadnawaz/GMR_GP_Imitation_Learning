...
class GmrBasedCoregionalize(Kern):
	...
	def _K_cython(self, B, X, X2=None):
		X = X[:, -1][:, None]
		if X2 is None:
			return coregionalize_cython.K_symmetric(B, np.asarray(X, dtype=int)[:,0])
		X2 = X2[:, -1][:, None]
		return coregionalize_cython.K_asymmetric(B, np.asarray(X, dtype=int)[:,0], np.asarray(X2, dtype=int)[:,0])


	def Kdiag(self, X):
		W = self._compute_weights(X)
		return np.diag(self.B)[np.asarray(X, dtype=int).flatten()] * W  # TODO check that it's right
